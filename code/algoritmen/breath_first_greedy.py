...
def print_branch_progress(battery: Battery, cheapest_config: Tuple[int, Tuple[int, ...]], progress_list: List[str]) -> None:
    """
    Appends progress information to a list when the cheapest configuration for a battery is found.
    ...
    - progress_list (List[str]): The list to which progress information will be appended.
    ...
    if cheapest_config:
        ...
        progress_list.append("Best amount of main branches: " + str(len(cheapest_config[1])))


def generate_partial_combinations(grid: Any, max_branches: int, progress_bar: Progress) -> Dict[Any, List[List[Any]]]:
    """
    Generate partial combinations for each battery in the grid.

    Parameters:
    - grid (Any): The grid object containing batteries and houses.
    - max_branches (int): The maximum number of branches for combination generation.

    Returns:
    Dict[Any, List[List[Any]]]: A dictionary mapping each battery to its partial combinations.
    """
...
def breath_first_greedy_fast(grid: Grid, max_branches: int, progress_bar: Progress) -> None:
    ...
    with multiprocessing.Manager() as manager:
        ...
        all_partial_combinations_per_battery = generate_partial_combinations(grid, max_branches, progress_bar)
